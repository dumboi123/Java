...
public class NhanVienDAL {
    ...
    public List<NhanVienDTO> getData(){
        try{
            String query = "SELECT * FROM `nhan_vien` WHERE `trang_thai` = 1";
    ...
    public void addData(String []arr){
        try {
            String query = "INSERT INTO `nhan_vien`(`ma_nhan_vien`, `ten_nhan_vien`, ngay_sinh, `sdt`, `mail`, `tai_khoan`,`trang_thai`) "
...
//            dataProvider.getStatement();
    ...
    public void delData(String ma_nhan_vien){
        try {
            String query = "Update `nhan_vien` Set `trang_thai` = 0 Where `ma_nhan_vien` = '"+ma_nhan_vien+"'";
//            dataProvider.getStatement();
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
    ...
    public void updateData(String []arr){
        try {
            String query = "UPDATE `nhan_vien` SET `ten_nhan_vien` = '"+arr[1]+"',`ngay_sinh` = '"+arr[2]+"', `sdt` = '"+arr[3]+"' "
                + " ,`mail` ='"+arr[4]+"' ,`tai_khoan` = '"+arr[5]+"' WHERE `ma_nhan_vien` = '"+arr[0]+"' and `trang_thai` = 1";
//            dataProvider.getStatement();
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
