...
public class NhaCungCapDAL {
    ...
    public void addData(String []arr){
        try {
            String query = "INSERT INTO `nha_cung_cap` (`ma_ncc`, `ten_ncc`, `dia_chi`,`trang_thai`) "
...
//            dataProvider.getStatement();
    ...
    public void delData(String ma_ncc){
        try {
            String query = "Update nha_cung_cap Set `trang_thai` = 0 Where `ma_ncc` = '"+ma_ncc+"'";
//            dataProvider.getStatement();
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
    ...
    public void updateData(String []arr){
        try {
            String query = "UPDATE `nha_cung_cap` "
                + "SET `ten_ncc`='"+arr[1]+"',`dia_chi`='"+arr[2]+"' WHERE `ma_ncc`='"+arr[0]+"' and `trang_thai` = 1";
//            dataProvider.getStatement();
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
