...
public class TaiKhoanDAL {
    ...
    public List<TaiKhoanDTO> getData(){
        ...
        try {
            String query = "SELECT * FROM `tai_khoan` WHERE `trang_thai` = 1";
...
//            dataProvider.getStatement();
    ...
    public void delData(int ma_tai_khoan){
        try {
            String query = "Update `tai_khoan` set `trang_thai` = 0 where `ma_tai_khoan` = '"+ma_tai_khoan+"'";
//            dataProvider.getStatement();
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
    ...
    public void updateData(String []arr){
        try {
            String query = "UPDATE `tai_khoan`"
                    + " SET `ten_tai_khoan` = '"+arr[1]+"',`mat_khau` = '"+arr[2]+"',`phan_quyen` = '"+arr[3]+"',`trang_thai` = '"+arr[4]+"'"
    ...
    public void updateUsernameAndPass(String []str){
        try {
            String query = "UPDATE `tai_khoan` SET"
                    + " `ten_tai_khoan` = '"+str[1]+"', `mat_khau` = '"+str[2]+"',`trang_thai` = 1"
...
//            dataProvider.getStatement();
    ...
    try {
        String query = "Select `ten_tai_khoan` From `tai_khoan` Where `trang_thai` = 1";
        ResultSet rs = dataProvider.excuteQuery(query);
        while (rs.next()) {
            if (username.equals(rs.getString("ten_tai_khoan"))) {
                return true;
            }
        }
    } catch (Exception e) {
