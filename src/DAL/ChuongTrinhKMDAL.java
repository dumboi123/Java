...
public class ChuongTrinhKMDAL {
    ...
    public  List<ChuongTrinhKMDTO> getData(){
        try{
            String query = "SELECT * FROM `chuong_trinh_khuyen_mai` WHERE `chi_so` = 1";
    ...
    public void addData(String []arr){
        try {
            String query = "INSERT INTO `chuong_trinh_khuyen_mai`(`ma_ctkm`, `ten_ctkm`, `ngay_bat_dau`, `ngay_ket_thuc`,`trang_thai`,`chi_so`) "
                + "VALUES ('"+arr[0]+"','"+arr[1]+"','"+arr[2]+"','"+arr[3]+"','0',1)";
//            dataProvider.getStatement();
    ...
    public void delData(int ma_ctkm){
        try {
            String query = "Update `chuong_trinh_khuyen_mai` Set `chi_so` = 0 Where `ma_ctkm` = '"+ma_ctkm+"'";
//            dataProvider.getStatement();
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
    ...
    public void updateData(String []arr){
        try {
            String query = "UPDATE `chuong_trinh_khuyen_mai` SET"
                ...
                + " WHERE `ma_ctkm`='"+arr[0]+"' and `chi_so` = 1";
//            dataProvider.getStatement();
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
