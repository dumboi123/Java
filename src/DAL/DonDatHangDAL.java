...
public class DonDatHangDAL {
    ...
    public  List<DonDatHangDTO> getData(){
        try{
            String query = "SELECT * FROM `don_dat_hang` WHERE `trang_thai` = 1";
    ...
    public void addData(String []str){
        try {
            ...
            String query = "INSERT INTO `don_dat_hang`(`ma_don_dat`, `ngay_dat`, `ma_ncc`, `tong_tien_dat`,`trang_thai`) "
            + "VALUES ('"+str[0]+"','"+date+"','"+str[1]+"','"+str[2]+"',1)";
//            dataProvider.getStatement();
    ...
    public void delData(String ma_don_dat){
        try {
            String query = "Update `don_dat_hang` Set `trang_thai` = 0 Where `ma_don_dat` = '"+ma_don_dat+"'";
//            dataProvider.getStatement();
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
