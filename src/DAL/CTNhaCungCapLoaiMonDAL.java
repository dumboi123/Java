...
public class CTNhaCungCapLoaiMonDAL {
    ...
    public  List<CTNhaCungCapLoaiMonDTO> getData(String ma_ncc){
        try{
            String query = "SELECT * FROM `chi_tiet_ncc_loai_mon` WHERE `ma_ncc` = '"+ma_ncc+"' and `trang_thai` = 1 "
                    + "ORDER BY `ma_ncc` ASC";
    ...
    public void addData(String []arr, StringBuilder tb){
        try {
            String query = "INSERT INTO `chi_tiet_ncc_loai_mon`(`ma_ncc`, `ma_loai`,`trang_thai`) "
                + "VALUES ('"+arr[0]+"','"+arr[1]+"',1)";
    ...
    public void delData(String []arr){
        try {
            String query = "Update chi_tiet_ncc_loai_mon Set `trang_thai` = 0 Where `ma_ncc` = '"+arr[0]+"' and `ma_loai` = '"+arr[1]+"'";
    ...
    public void updateData(String[]str,StringBuilder t){
        try {
            String query = "UPDATE `chi_tiet_ncc_loai_mon` "
                    + "SET `ma_loai`='"+str[2]+"' WHERE  `ma_ncc`= '"+str[0]+"' AND `ma_loai` = '"+str[1]+"' and `trang_thai` = 1";
            int kq = dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
            if(kq == 0){
