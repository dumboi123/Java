...
public class PhieuGiamGiaDAL {
    ...
    public List<PhieuGiamGiaDTO> getData(){
        try{
            String query = "SELECT * FROM `phieu_giam_gia` WHERE `chi_so` = 1";
    ...
    public void addData(String []arr){
        try {
            String query = "INSERT INTO`phieu_giam_gia`(`ma_phieu_giam_gia`, `ten_phieu_giam_gia`, `giam`, `ngay_bat_dau`, `ngay_ket_thuc`, `trang_thai`,`chi_so`) "
    ...
    public void delData(String ma_phieu_giam_gia){
        try {
            String query = "Update `phieu_giam_gia` Set `chi_so` = 0 Where `ma_phieu_giam_gia` = '"+ma_phieu_giam_gia+"'";
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void updateData(String []arr){
        try {
            String query = "UPDATE `phieu_giam_gia` SET `ten_phieu_giam_gia` = '"+arr[1]+"', `giam` = '"+arr[2]+"', `ngay_bat_dau` = '"+arr[3]+"', `ngay_ket_thuc` = '"+arr[4]+"' "
                + " WHERE `chi_so` = 1 and `ma_phieu_giam_gia` = '"+arr[0]+"'";
            dataProvider.ExecuteUpdate(query);
            dataProvider.closeConnect();
        } catch (Exception e) {
            e.printStackTrace();
